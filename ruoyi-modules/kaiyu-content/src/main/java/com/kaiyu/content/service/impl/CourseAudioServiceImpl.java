...
public class CourseAudioServiceImpl implements ICourseAudioService {
    ...
    @Override
    public CourseAudioDto getAudioJsonByCourseId(Long courseId,Long episode) {
...
//            KaiYuEducationException.cast(CommonError.REQUEST_NULL);
        ...
        if (StringUtils.isNotEmpty(courseAudioJson)) {
            ...
            //缓存数据
            redisTemplate.opsForHash().put("getAudioJsonByCourseId",courseAudioKey,JSON.toJSONString(courseAudio));
            //设置过期时间
...
//            KaiYuEducationException.cast("课程id和课程集不能为空");
       ...
       if (courseAudio !=null){
           ...
           courseAudioMapper.updateById(courseAudio);
...
//            KaiYuEducationException.cast("课程id和课程集不存在");
        ...
        if (filter_data.isEmpty()){
            //删除此课程脚本
            courseAudioMapper.deleteById(courseAudio.getId());
        }else {
            // 更新audiojson字段
