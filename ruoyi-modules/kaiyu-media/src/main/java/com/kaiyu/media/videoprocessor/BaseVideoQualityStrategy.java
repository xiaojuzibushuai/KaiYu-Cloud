...
//            System.out.println(s);
        ...
        //替换敏感信息
        String m3u8FilePath =  (targetVideoDir+"/encrypted_slice.m3u8").replace("\\","/");
        try {
            // 读取原始M3U8文件内容
            ...
            String modifiedContent = originalContent.toString()
                    .replaceAll("URI=\"[^\"]+\"", "URI=\"SECRET\"");

            // 可以添加更多替换规则，比如替换 IV 信息
            // modifiedContent = modifiedContent.replaceAll("IV=[^, \n]+", "IV=0x00000000000000000000000000000000");

            BufferedWriter writer = new BufferedWriter(new FileWriter(m3u8FilePath));
            writer.write(modifiedContent);
            writer.close();

        } catch (IOException e) {
            log.info("替换m3u8文件敏感信息失败");
            return null;
        }
